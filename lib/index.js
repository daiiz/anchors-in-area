...
var AnchorsInArea = function () {
  ...
  _createClass(AnchorsInArea, [{
    ...
    value: function findRelative(range) {
      var tagName = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'a';

      var anchors = this.find(range, tagName);
    ...
    value: function find(range) {
      ...
      var candidateAnchorNodes = document.querySelectorAll(tagName);
      ...
      try {
        for (var _iterator3 = candidateAnchorNodes[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
          ...
          var link = anchorNode.href || anchorNode.src;
          ...
          var rectLeft = rect.left + this.sumPx(anchorNode, ['padding-left', 'border-left-width']);
          var rectRight = rect.right - this.sumPx(anchorNode, ['padding-right', 'border-right-width']);
          var rectTop = rect.top + this.sumPx(anchorNode, ['padding-top', 'border-top-width']);
          var rectBottom = rect.bottom - this.sumPx(anchorNode, ['padding-bottom', 'border-bottom-width']);
    ...
    key: 'sumPx',
    value: function sumPx(elem, attrs) {
      var sum = 0;
      ...
      try {
        for (var _iterator4 = attrs[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
          ...
          var px = (0, _jquery2.default)(elem).css(attr);
          sum += +px.split('px')[0] || 0;
        }
      } catch (err) {
    ...
    key: 'getAnchors',
    value: function getAnchors(rangeStr) {
      return (0, _simple.getAnchors)(rangeStr);
    }
  }]);
